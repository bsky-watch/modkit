...
func init() {
	...
	if plcAddr != "" {
		fr.resolvers = append(fr.resolvers, &api.PLCServer{Host: plcAddr})
	}
	fr.resolvers = append(fr.resolvers, &api.PLCServer{Host: "https://plc.directory"})

	resolver.AddHandler("plc", fr)
